...
---- REV DATE: 6/AUG/2019 - 9/SEPT/2020 Marcela Torres
...
---- Used MozART_q3_2019_Clean to compare FY19Q1 Evaluation_Date = '2018-12-21' and FY19Q2 Evaluation_Date = '2019-03-21')
...
(
	...
	,datatarv_second, proximatarv_second, Second_DayswithARV
	 ...
	 ,CASE	WHEN [datainiciotarv] is not null AND [first_datatarv] is not null AND cast([datainiciotarv] AS DATE)<=cast([first_datatarv] AS DATE) THEN cast([datainiciotarv] AS DATE)
		...
		CurrentAge=(datediff(mm,datanasc,'2018-12-21')/12.0)
	FROM
	...
	*/
	...
	FROM
		...
		FROM t_tarv tv
		LEFT JOIN
		(SELECT nid, Evaluation_Date = '2018-12-21', AccessFilePath
		FROM t_paciente) tpo
...
-----rename table
SELECT *
INTO Sandbox.dbo.TX_CURR_FY19Q1_C
FROM CTE2
WHERE datainiciotarv >= '2012-01-01'
/*ORDER BY nid, Outcome_FY20  desc*/


UPDATE Sandbox.dbo.TX_CURR_FY19Q1_C
SET Evaluation_Date='2018-12-21'
...
Update Sandbox.dbo.TX_CURR_FY19Q1_C			/*****check how JEMBI handles cases without datasaida tarv****/
...
FROM Sandbox.dbo.TX_CURR_FY19Q1_C
...
(
	...
	,datatarv_second, proximatarv_second, Second_DayswithARV
	 ,dataseguimento_second, proximaconsult_second /*, person.Gravidez*/
     ,CurrentAge=(datediff(mm,datanasc,'2019-03-21')/12.0)
	 ,CASE	WHEN [datainiciotarv] is not null AND [first_datatarv] is not null AND cast([datainiciotarv] AS DATE)<=cast([first_datatarv] AS DATE) THEN cast([datainiciotarv] AS DATE)
		WHEN [datainiciotarv] is not null AND [first_datatarv] is not null AND cast([datainiciotarv] AS DATE)>cast([first_datatarv] AS DATE) THEN cast([first_datatarv] AS DATE)
		WHEN [first_datatarv] is null and [datainiciotarv] is not null THEN cast([datainiciotarv] AS DATE)
		WHEN [first_datatarv] is not null and [datainiciotarv] is null THEN cast([first_datatarv] AS DATE)
	...
	*/
	...
	FROM
		...
		FROM t_tarv tv
		LEFT JOIN
		(SELECT nid, Evaluation_Date = '2019-03-21', AccessFilePath
		FROM t_paciente) tpo
		ON tv.nid = tpo.nid AND tv.AccessFilePath = tpo.AccessFilePath
		WHERE cast(datatarv AS date) <= Evaluation_Date
		) ntv
...
-----rename table
SELECT *
INTO Sandbox.dbo.TX_CURR_FY19Q2_D
FROM CTE2
WHERE datainiciotarv >= '2012-01-01'
...
UPDATE Sandbox.dbo.TX_CURR_FY19Q2_D
SET Evaluation_Date='2019-03-21'
Where Evaluation_Date is null
...
Update Sandbox.dbo.TX_CURR_FY19Q2_D	/*****check how JEMBI handles cases without datasaida tarv****/
SET Outcome_FY19='LTFU' WHERE Outcome_FY19 is null AND datasaidatarv is NULL AND 
((Max_dataproxima_tarv < dateadd(dd,-28,Evaluation_Date)) OR 
(Max_dataproximaconsult < dateadd(dd, -28, Evaluation_Date)))
...
Update Sandbox.dbo.TX_CURR_FY19Q2_D
SET Outcome_FY19='Abandon' WHERE Outcome_FY19 is null AND TX_CURR_FY19 ='Not Active' AND codestado='ABANDONO' AND datasaidatarv <= Evaluation_Date AND (Max_datatarv<Evaluation_Date OR Max_dataseguimento<Evaluation_Date)

Update Sandbox.dbo.TX_CURR_FY19Q2_D
SET Outcome_FY19='ART Suspend' WHERE Outcome_FY19 is null AND TX_CURR_FY19 ='Not Active' AND codestado ='SUSPENDER TRATAMENTO' AND datasaidatarv <= Evaluation_Date AND (Max_datatarv<Evaluation_Date OR Max_dataseguimento<Evaluation_Date)
...
FROM Sandbox.dbo.TX_CURR_FY19Q2_D

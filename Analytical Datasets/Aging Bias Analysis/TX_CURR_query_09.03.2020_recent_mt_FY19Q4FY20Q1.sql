	...
	(     /*case 1 no exit, just compare dates*/
		(cast(revised_datainiciotarv AS DATE) <Evaluation_Date) AND
				...
				((datasaidatarv IS NULL) AND
						...
						(Max_dataproxima_tarv >= dateadd(dd,-30,Evaluation_Date)) OR 
						(Max_dataproximaconsult >= dateadd(dd, -30, Evaluation_Date))
				)
...
-----rename table
SELECT *
INTO Sandbox.dbo.TX_CURR_FY19Q4
FROM CTE2
WHERE datainiciotarv >= '2012-01-01'
/*ORDER BY nid, Outcome_FY20  desc*/


UPDATE Sandbox.dbo.TX_CURR_FY19Q4
...
Update Sandbox.dbo.TX_CURR_FY19Q4			/*****check how JEMBI handles cases without datasaida tarv****/
...
-----rename table
SELECT *
INTO Sandbox.dbo.TX_CURR_FY20Q1
FROM CTE2
WHERE datainiciotarv >= '2012-01-01'
...
UPDATE Sandbox.dbo.TX_CURR_FY20Q1
...
Update Sandbox.dbo.TX_CURR_FY20Q1	/*****check how JEMBI handles cases without datasaida tarv****/

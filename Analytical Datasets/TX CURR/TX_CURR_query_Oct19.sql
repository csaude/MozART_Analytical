...
(
	...
	FROM
		...
		FROM t_tarv tv
		LEFT JOIN
		(SELECT nid, Evaluation_Date = '2018-10-21', AccessFilePath
		FROM t_paciente) tpo
...
( 
	...
	/*((datasaidatarv IS NULL) /*tem que vir um end*/ OR (datasaidatarv > Evaluation_Date) OR ((datasaidatarv < Evaluation_Date) AND ((Max_datatarv> datasaidatarv) AND (Max_datatarv< Evaluation_Date)))) AND */
	/*((Max_datatarv >= dateadd(dd,-30,Evaluation_Date) AND Max_datatarv <= Evaluation_Date) OR (Max_dataseguimento >= dateadd(dd, -30, Evaluation_Date) AND Max_dataseguimento <= Evaluation_Date) OR (dataproxima >=Evaluation_Date))*/ 
),
CTE2 AS
( 
	SELECT *, CASE WHEN TX_CURR_OCT = 'Not Active' AND ((codestado = 'ABANDONO') OR (codestado IS NULL) AND (datasaidatarv < Evaluation_Date)) THEN 'LTFU'
	WHEN TX_CURR_OCT = 'Not Active' AND ((codestado = 'TRANSFERIDO PARA') AND (datasaidatarv < Evaluation_Date)) THEN 'Transferred Out'
	WHEN TX_CURR_OCT = 'Not Active' AND ((codestado = 'OBITO') AND (datasaidatarv < Evaluation_Date)) THEN 'Dead'
	WHEN TX_CURR_OCT = 'Not Active' AND ((codestado IS NULL)) THEN 'LTFU'
	WHEN TX_CURR_OCT = 'Active' Then 'Active'
	ELSE 'LTFU'
	END AS [Outcome_Oct]
	FROM CTE1)

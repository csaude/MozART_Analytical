...
(
       ...
       dateadd(mm, 12, cast(datainiciotarv as date)) as Outcome_Date_Twelvem, person.datasaidatarv as Exit_Date, person.codestado as Last_Status,
       tt.Max_datatarv as Last_Drug_Pickup_Date_Twelvem, tt.dataproxima as Next_Drug_Pickup_Date_Twelvem,
       ss.Max_dataseguimento as Last_Consultation_Date_Twelvem, ss.dataproximaconsulta as Next_Consultation_Date_Twelvem, person.AccessFilePath
       ...
       LEFT JOIN
       (SELECT * FROM(
       SELECT ROW_NUMBER() OVER (PARTITION BY ntv.nid, ntv.AccessFilePath ORDER BY ntv.dataproxima desc) as rownum, ntv.nid, ntv.AccessFilePath, ntv.Outcome_Date_Twelvem, ntv.datatarv as Max_datatarv, ntv.dataproxima
       FROM
              (
              SELECT tv.nid, tv.AccessFilePath, cast(datatarv as date) as datatarv, cast(dataproxima as date) as dataproxima, tpo.Outcome_Date_Twelvem
              FROM t_tarv tv
              LEFT JOIN
              (SELECT nid, Outcome_Date_Twelvem = dateadd(mm, 12, cast(datainiciotarv as date)), AccessFilePath
              FROM t_paciente) tpo
              ON tv.nid = tpo.nid AND tv.AccessFilePath = tpo.AccessFilePath
              WHERE datatarv <= Outcome_Date_Twelvem
              ) ntv
       ) t
       WHERE t.rownum = '1') tt
       ...
       FROM 
              (
              SELECT ts.nid, ts.AccessFilePath, cast(dataseguimento as date) as dataseguimento, cast(dataproximaconsulta as date) as dataproximaconsulta, tpo1.Outcome_Date_Twelvem
              FROM t_seguimento  ts
              LEFT JOIN
              (SELECT nid, Outcome_Date_Twelvem = dateadd(mm, 12, cast(datainiciotarv as date)), AccessFilePath
              FROM t_paciente) tpo1
              ON ts.nid = tpo1.nid AND ts.AccessFilePath = tpo1.AccessFilePath
              WHERE dataseguimento <= Outcome_Date_Twelvem
              ) nts
       ) s
       WHERE s.rownum = '1') ss
...
( 
       SELECT *, 
       CASE 
       WHEN Outcome_Date_Twelvem > '2/10/2020' THEN 'Not Evaluated'
       WHEN 
       (
              (Exit_Date < Last_Drug_Pickup_Date_Twelvem) OR 
              (Exit_Date IS NULL) OR 
              (Exit_Date < Last_Consultation_Date_Twelvem) OR
              (Exit_Date > Outcome_Date_Twelvem)
       ) AND
       (    
			   Last_Drug_Pickup_Date_Twelvem > dateadd(mm,-3,Outcome_Date_Twelvem) OR 
			   Next_Drug_Pickup_Date_Twelvem > dateadd(mm,-2,Outcome_Date_Twelvem) OR 
			   Last_Consultation_Date_Twelvem > dateadd(mm,-3,Outcome_Date_Twelvem) OR
			   Next_Consultation_Date_Twelvem > dateadd(mm,-3,Outcome_Date_Twelvem) 
...
( 
       SELECT *, 
       CASE WHEN Retained_Status_12m = 'Not Retained' AND ((Last_Status = 'ABANDONO') OR (Last_Status IS NULL) AND (Exit_Date < Outcome_Date_Twelvem)) THEN 'LTFU'
       WHEN Retained_Status_12m = 'Not Retained' AND ((Last_Status = 'TRANSFERIDO PARA') AND (Exit_Date < Outcome_Date_Twelvem)) THEN 'Transferred Out'
       WHEN Retained_Status_12m = 'Not Retained' AND ((Last_Status = 'OBITOU') AND (Exit_Date < Outcome_Date_Twelvem)) THEN 'Dead'
...
SELECT *
INTO Sandbox.dbo.Retention_12m_2017_FinalM
FROM CTE2
WHERE Cohort_Year = '2017' AND 
Initiation_Date IS NOT NULL AND
first_datatarv<= dateadd(mm,6, Initiation_Date)
--(Last_Drug_Pickup_Date_Twelvem IS NULL OR Last_Drug_Pickup_Date_Twelvem >= Initiation_Date) AND
...
---change lines 118 and 113 and 116 and 117 to use scheduled pick up and schedule consultation

---- =========================================================================================================
---- WORKING SQL QUERY FOR 3M RETENTION DATASET PRODUCTION
---- BASED ON CDC MOZAMBIQUE RETENTION DATA TEMPLATE
---- AUTHOR: RANDY YEE (CDC/GDIT)
---- REV DATE: 9/8/2019
---- Retention Definition: All patients who have either a clinical consultation or drug pick up between day 61 and day 120
...
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID('[dbo].[RetentionGenerator_3m]') AND type IN ('P', 'PC', 'RF', 'X'))
  DROP PROCEDURE [dbo].[RetentionGenerator_3m]
GO

IF OBJECT_ID('[Sandbox].[dbo].[Retention_3m]', N'U') IS NOT NULL
  DROP TABLE [Sandbox].[dbo].[Retention_3m]
GO

CREATE PROCEDURE [dbo].[RetentionGenerator_3m] @CreationDate Date
AS
...
(
	...
	dateadd(dd, 120, cast(datainiciotarv as date)) as Outcome_Date, person.datasaidatarv as Exit_Date, person.codestado as Last_Status,
	...
	FROM
		...
		FROM t_tarv tv
		LEFT JOIN
		(SELECT nid, Outcome_Date = dateadd(dd, 120, cast(datainiciotarv as date)), AccessFilePath
		FROM t_paciente) tpo
...
( 
	...
	(	
		((Last_Drug_Pickup_Date BETWEEN dateadd(dd,-61, Outcome_Date) AND Outcome_Date) AND Last_Drug_Pickup_Date!= Initiation_Date) OR 
		((Last_Consultation_Date BETWEEN dateadd(dd,-61, Outcome_Date) AND Outcome_Date) AND Last_Consultation_Date!= Initiation_Date)
	) THEN 'Retained'
	ELSE 'Not Retained'
...
( 
	SELECT *, 
	CASE WHEN Retained_Status_3m = 'Not Retained' AND ((Last_Status = 'ABANDONO') OR (Last_Status IS NULL) AND (Exit_Date < Outcome_Date)) THEN 'LTFU'
	WHEN Retained_Status_3m = 'Not Retained' AND ((Last_Status = 'TRANSFERIDO PARA') AND (Exit_Date < Outcome_Date)) THEN 'Transferred Out'
	WHEN Retained_Status_3m = 'Not Retained' AND ((Last_Status = 'OBITOU') AND (Exit_Date < Outcome_Date)) THEN 'Dead'
	WHEN Retained_Status_3m = 'Not Retained' AND ((Last_Status IS NULL)) THEN 'LTFU'
	WHEN Retained_Status_3m = 'Retained' THEN 'Retained'
	WHEN Retained_Status_3m = 'Not Evaluated' THEN 'Not Evaluated'
	ELSE 'LTFU'
	END AS [Outcome_3m]
	FROM CTE1
)
SELECT *
INTO Sandbox.dbo.Retention_3m
FROM CTE2
